...
class OASIS extends Catalog implements Interfaces\ImageSearchInterface {
    ...
    private function getSearchUrl($keyword, $desiredResults, $startIndex)
    {
        return str_replace(
            array('{keyword}', '{start-index}', '{count}'),
            array(urlencode($keyword), $startIndex, $desiredResults), 
            self::SEARCH_URL_PATTERN
        );
    }

    private function getImagesFromXpath($xpath)
    {
        $results = array();
        $resultNodes = $xpath->query('//doc');
    ...
    private function areMoreLeft($totalImages, $desiredResults, $startIndex)
    {
        return $startIndex + $desiredResults < $totalImages;
    }

    public function fetchResultsFlat($keyword, $desiredResults = 25, $startIndex = 0)
    {
        $images      = array();
        $searchUrl   = $this->getSearchUrl($keyword, $desiredResults, $startIndex);

        $xpath = $this->fetchXpath($searchUrl);

        $totalImages = (int) $xpath->query('//result')->item(0)->getAttribute('numFound');

        $images = array_merge($images, $this->getImagesFromXpath($xpath));

        while (count($images) < $desiredResults &&
          $startIndex + $desiredResults < $totalImages) {
            $startIndex += $desiredResults - 1;
            $searchUrl = $this->getSearchUrl($keyword, $page);
            $xpath = $this->fetchXpath($search_url);
            $images = array_merge($images, $this->getImagesFromXpath($xpath));
        }

        $more = $this->areMoreLeft($totalImages, $desiredResults, $startIndex);

...
class ProjectsController extends BaseController
{
    ...
    public function postProjectAction()
    {
        ...
        $homepageTag = new Tag();
        $homepageTag->setName('homepage');
        $project->addTag($homepageTag);
    ...
    public function putProjectsAction($projectId)
    {
        ...
        if( $this->getRequest()->request->has('cover_image') ) {
            ...
            if( $project->getCoverImage() != $this->getRequest()->request->get('cover_image') ){
                ...
                $logger->info('New URL: '. $coverImage);
        ...
        if( $this->getRequest()->request->has('autoplay') ) {
            $project->setAutoplay( $this->getRequest()->request->get('autoplay') );
        }

        if( $this->getRequest()->request->has('show_metadata') ) {
            $project->setShowMetadata( $this->getRequest()->request->get('show_metadata') );
        }

        if( $this->getRequest()->request->has('autoplay_duration') ) {
            $project->setAutoplayDuration( $this->getRequest()->request->get('autoplay_duration') );
        }
    ...
    public function putProjectItemsAction($projectId, Request $request) {
        $user = $this->get('security.context')->getToken()->getUser();
        $dm = $this->get('doctrine_mongodb')->getManager();        
        $project = $dm->getRepository('ZeegaDataBundle:Project')->findOneById($projectId);
        ...
        // Do auth here
        if( !isset($user) || $project->getUser() != $user) {
            return parent::getStatusResponse(401);
        }
        if ( !$project ) {
            throw $this->createNotFoundException('Unable to find the Project with the id ' + $projectId);
        }
        // For each item, add a frame on the project, add a layer to the frame, add an item to the layer
        // TODO: Add items to upload pool
        foreach ($items as $item) {
            ...
            if (isset($item['thumbnail_url'])) {
                $frame->setThumbnailUrl($item['thumbnail_url']);
            }
            ...
            $layer->setAttr($item);
            $layer->setType($item["layer_type"]);
            $project->addLayer($layer);

            // get the sequence and update the frames
            $sequence = $project->getSequences()->first();
            $sequenceFrames = $sequence->getFrames();
            array_push($sequenceFrames, (string)$frameId);
            $sequence->setFrames($sequenceFrames);

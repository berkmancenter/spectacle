...
function( app, ControlsView ) {
    ...
    */
    return app.Backbone.Layout.extend({
        ...
        initialize: function() {
            if (!this.model.get('windowRatio')) {
                var winWidth = app.$( this.model.get("target") ).find(".ZEEGA-player").width(),
                    winHeight = app.$( this.model.get("target") ).find(".ZEEGA-player").height();

                if (this.model.zeega.getCurrentProject().attributes.show_metadata) {
                  winHeight -= 110;
                }
                var screenRatio = winWidth / winHeight;
                this.model.set({windowRatio: screenRatio});
            }
        ...
        onClick: function() {
            if (this.model.get('autoplay') && this.model.state == 'playing') {
                this.model.pause();
            }
        },
        ...
        getWrapperSize: function() {
            ...
            playerMaxWidth = winHeight * screenRatio;

    ...
    return app.Backbone.Model.extend({
        ...
        defaults: {
            ...
            **/
            ...
            /**
            Sets the duration between pages when autoplaying
            ...
            **/
            ...
            If false, fits aspect ratio of window

            @property windowRatio
            @type Float or Boolean
            @default 4/3
            **/
            windowRatio: 0.75,
        ...
        play: function() {
            ...
            if ( !this.ready ) {
                ...
                if (this.get('autoplay')) {
                    this.resetPageInterval();
                }
                this.emit("player:play", this );
            }
        },
        ...
        setPageInterval: function() {
            this.set(
                'autoplayIntervalId',
                setInterval(this.cueNext, this.get('autoplayDuration'))
                );
        },

        clearPageInterval: function() {
            if (this.get('autoplayIntervalId'))
                clearInterval(this.get('autoplayIntervalId'));
        },

        resetPageInterval: function() {
            this.clearPageInterval();
        ...
        cueNext: function() {
            var nextPage = app.player.zeega.getNextPage();
            //var nextPage = this.zeega.getNextPage();

            if ( nextPage ) {
                app.player.cuePage( nextPage );
                app.player.emit('player:next', app.player);
            }
        },

        // goes to the prev frame after n ms
        cuePrev: function( ms ) {
            var prevPage = app.player.zeega.getPreviousPage();

            if ( prevPage ) {
                app.player.cuePage( prevPage );
                app.player.emit('player:prev', app.player);
            }
        },
